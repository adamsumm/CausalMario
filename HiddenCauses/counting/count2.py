...
with open(sys.argv[1],'r') as openfile:
    ...
    for line in openfile:
        ...
        ta = dat[5]
        dv = dat[6]
        if t != 'None' and not (oA in filtered or oB in filtered or s in filtered):
            kk += 1

            sameDiffA = s
            sameDiffB = oB
            if t == 'VelChange' or t == 'Delete':
                if oA == s:
                    sameDiffA = True
                else:
                    sameDiffA = False
            ...
            if oA not in counts:
                counts[oA] = {}
            acause = (oB==s,A2B)#(sameDiffB,A2B)
            effect = (t,s==oA,ta,dv)#(t,sameDiffA,ta,dv)#
            ...
            bcause = (oA==s,opposite[A2B])
            effect = (t,s==oB,ta,dv)
            #bcause = (sameDiffA,opposite[A2B])#(oB==s,A2B)
            #effect = (t,sameDiffB,ta,dv)#(t,s==oA,ta,dv)
            if oB not in counts:
                counts[oB] = {}
...
for o in pmi:
    for ce in pmi[o]:
        ...
        if npmi[o][ce] > threshold:#if pmi[o][ce] > threshold:
...
for o in objects:
    if o not in coveredObjects:
        notCovered.append( o)
print len(notCovered),len(coveredObjects)
print len(clusters)
