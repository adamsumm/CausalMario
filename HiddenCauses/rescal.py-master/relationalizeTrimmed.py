...
fname = '{}{}_trimmed.pkl'.format(sys.argv[1],sys.argv[2])

if os.path.isfile(fname) :
    A,R,fit,itr,exectimes = pickle.load(open(fname,'r'))
else:
    A, R, fit, itr, exectimes = rescal_als(sparse, int(sys.argv[2]), init='nvecs', conv=1e-7, lambda_A=1, lambda_R=1)
...
fname = '{}{}_{}norm_trimmed.pkl'.format(sys.argv[1],sys.argv[2],sys.argv[3])
...
for e in e2vec:
    print e, e2vec[e]
    groups = (e2vec[e]>float(sys.argv[4])).nonzero()
    for g in groups[0]:
        if g not in membership:
            membership[g] = set()
        membership[g].add(e)
